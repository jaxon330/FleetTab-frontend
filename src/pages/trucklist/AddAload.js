...
function AddALoad(
  {
    addALoadForm, closeAddALoadForm, showAddALoadForm, truckInfo, drivers, loads, setLoads, addLoad
  }) {
    ...
    let truckDetails = async () => {
      let truckData = await fetch('http://localhost:4000/' + truckInfo._id)
      let json = await truckData.json()
      console.log(json);
      if (json) {
        setNewTruck(json)
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        let responce = await fetch('http://localhost:4000/edit/' + truckInfo._id, {
            method: 'PUT',
            body: JSON.stringify({
    ...
    <>
      ...
      <Form onSubmit={handleSubmit}>
          <Modal.Header closeButton>
            <Modal.Title id="order-food">Add A Load</Modal.Title>
          </Modal.Header>
          <Modal.Body>
